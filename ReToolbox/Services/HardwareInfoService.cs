...
{
    // Hardware information gathered via WMI + DXGI. Ported from tubatools
    // (https://github.com/luolangaga/tubatools), with CPU-Z calibration,
    // disk-temperature enrichment and external settings coupling removed.
    public static class HardwareInfoService
    {
        ...
        private static extern bool EnumDisplaySettings([MarshalAs(UnmanagedType.LPWStr)] string? lpszDeviceName, int iModeNum, ref DEVMODE lpDevMode);
        ...
        private static string FormatDisks()
        {
            ...
            return string.Join(Separator, disks);
        }
        ...
        private static string FormatDisplays()
        {
            ...
            return string.Join(Separator, monitorInfos.Select(mi =>
            {
